...
using just_val1_sender_t = decltype(ex::just(val_type1{0}) | ex::transfer(Scheduler{}));
...
namespace custom {
  ...
  auto tag_invoke(
      ex::transfer_t,
      custom::domain,
      just_val1_sender_t<custom_inline_scheduler> sndr,
      ex::receiver_from<just_val1_sender_t<inline_scheduler>> auto rcvr) {
    return ex::connect(
      ex::just(val_type1{53}) | ex::transfer(inline_scheduler{}),
      std::move(rcvr));
  }
}
}
...
TEST_CASE("transfer can be customized", "[adaptors][transfer]") {
  ...
  auto snd = ex::just(val_type1{1}) | ex::transfer(custom_inline_scheduler{});
  val_type1 res{0};
