...
ex::sender auto handle_classify_request(const http_request& req) {
  return
      ...
      | ex::then(do_classify)
      // handle errors
      // | ex::upon_error(on_classification_error) // TODO: make this work
      // handle cancellation
      // | ex::upon_done(on_classification_cancelled) // TODO: make this work
...
int main() {
  ...
  for (int i = 0; i < 12; i++) {
    ...
    ex::sender auto action = std::move(snd) | ex::then([](http_response resp) {
      std::cout << "Sending response: " << resp.status_code_ << " / " << resp.body_ << "\n";
    });
    ex::start_detached(std::move(action));
  }
